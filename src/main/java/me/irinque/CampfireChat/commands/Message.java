            ...
            if (player_receiver != null)
            {
                ...
                if (message_content.size() > 1)
                {
                    for (int i = 1; i < message_content.size(); i++)
                    {
                        ...
                        if (i < message_content.size() - 1) {string_builder.append(" ");}
                    }
                    ...
                    if (plugin.getConfig().getString("custom-private-message.notification").equals("true")) {player_receiver.playSound(player_receiver.getLocation(), Sound.BLOCK_NOTE_BLOCK_BELL, 1, 1);}
                }
                else
                {
